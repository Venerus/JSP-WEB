...
<body>
	...
	<div class="main">
		...
		<div id="pageTitle"><span>Tip/기록 수정</span></div>
            ...
            <table>
                <tr class="rowCell">
                    <td rowspan="4" id="tipPic" style="background-image:url(/project/pic/${dto.picture}); background-repeat: no-repeat; background-position: center center; padding: 5px; vertical-align:top;">
	                    <c:if test="${dto.allowSharing eq 'N'}">
	                    	<button type="button" id="share" class="btn btn-success" onclick="location.href='/project/mypage/sharingok.do?tourPlanSeq=${tourPlanSeq}';">공유</button>
	                    </c:if>
	                    <c:if test="${dto.allowSharing eq 'Y'}">
	                    	<button type="button" id="shareX" class="btn btn-danger" onclick="location.href='/project/mypage/sharingcancel.do?tourPlanSeq=${tourPlanSeq}';">공유취소</button>
	                    </c:if>
                    </td>
